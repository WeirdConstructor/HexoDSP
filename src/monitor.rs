...
const MONITOR_INPUT_LEN_PER_SAMPLE: usize =
    (MONITOR_MINMAX_LEN_S * IMAGINARY_MAX_SAMPLE_RATE) / MONITOR_MINMAX_SAMPLES;
...
impl MonitorMinMax {
    ...
    pub fn process(&mut self, mon_buf: &mut MonitorBufPtr) -> bool {
        ...
        while let Some(sample) = mon_buf.next_sample_for_signal(self.sig_idx) {
            ...
            if self.cur_min_max.2 >= MONITOR_INPUT_LEN_PER_SAMPLE {
                self.buf[self.buf_write_ptr] = (self.cur_min_max.0, self.cur_min_max.1);
                new_data = true;
...
impl MinMaxMonitorSamples {
    pub fn new() -> Self {
        Self { samples: [(0.0, 0.0); MONITOR_MINMAX_SAMPLES], buf_ptr: 0 }
    }
...
impl Monitor {
    pub fn new(
        ...
        let monitor_samples = Arc::new(Mutex::new([MinMaxMonitorSamples::new(); MON_SIG_CNT]));
        ...
        let th = std::thread::spawn(move || {
            ...
            loop {
                ...
                if proc.check_new_data() {
                    let mut ms =
                        th_mon_samples.lock().expect("Unpoisoned Lock for monitor_samples");
    ...
    pub fn get_minmax_monitor_samples(&mut self, idx: usize) -> &MinMaxMonitorSamples {
        ...
        if self.new_data.load(std::sync::atomic::Ordering::Relaxed) {
            let ms = self.monitor_samples.lock().expect("Unpoisoned Lock for monitor_samples");

            pt.print("XXX");

            for i in 0..MON_SIG_CNT {
                ms[i].copy_to(&mut self.monitor_samples_copy[i]);
            }
...
impl MonitorProcessor {
    pub fn new(
        ...
        Self { rb_mon_con, rb_recycle_prod, procs, new_data: false }
    }
...
pub fn new_monitor_processor() -> (MonitorBackend, Monitor) {
    ...
    let backend = MonitorBackend { rb_mon_prod, rb_recycle_con, unused_monitor_buffers };
...
mod tests {
    ...
    fn send_n_monitor_bufs(backend: &mut MonitorBackend, first: f32, last: f32, count: usize) {
        for _ in 0..count {
    ...
    fn wait_for_monitor_process() {
        ...
        //        the new_data flag!
        std::thread::sleep(std::time::Duration::from_millis(3 * MONITOR_PROC_THREAD_INTERVAL_MS));
    }
    ...
    fn check_monitor_proc() {
        ...
        let count1 = (MONITOR_INPUT_LEN_PER_SAMPLE / MAX_BLOCK_SIZE) + 1;
        let count2 = 2 * ((MONITOR_INPUT_LEN_PER_SAMPLE / MAX_BLOCK_SIZE) + 1);
        ...
        assert_eq!(backend.count_unused_mon_bufs(), MONITOR_BUF_COUNT - count1 - count2);
    ...
    fn check_monitor_wrap_buf() {
        ...
        assert_eq!((sl[MONITOR_MINMAX_SAMPLES - 1].1 * 10000.0).floor() as u32, 9937);
